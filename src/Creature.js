...
let Creature = (function() {
    ...
    function Creature(spec) {
        ...
        if (arguments.length !== 1) {
            throw `Creature constructor expects 1 argument, passed ${arguments.length} arguments`;
        }

        if (m_amount === undefined) {
            throw "missing money amount in creature";
        }
        ...
        return new Proxy(this, {
            get: function(target, name) {
                ...
                if (!(name in target.attr) && !(name in target.craft)) {
                    throw new Error('You can only access attributes in Creature class; non existent attribute "' + name + '"');
                }
